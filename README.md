...
```powershell
Get-Content .\migration_author_id.sql | .\mysql.exe -u root -p dblib
```

For an existing database, add physical copy counts with:

```powershell
Get-Content .\migration_copy_counts.sql | .\mysql.exe -u root -p dblib
```

Load the demo catalogue and accounts (optional):

```powershell
Get-Content .\seed.sql | .\mysql.exe -u root -p dblib
```
